...
class wst_server_reverse {
  constructor(port){
   this.port = port;
    ...
    log4js.addAppender(log4js.appenders.file('../etc/log/wst_reverse_server_onport_'+this.port+'.log'));  
    ...
    this.wsServerForControll.on('request', (function(_this){
      return function(request){
        ...
        if (uri.query.dst != undefined){
          ...
          //LOG MESSAGE
          _this.logger.info('WS Connectio for Control Created');
          request.wsConnectionForControll.on('close', function(reasonCode, description) {
            _this.logger.info('WebSocket Controll Peer ' + request.wsConnectionForControll.remoteAddress + ' disconnected for:"'+description+'"');
