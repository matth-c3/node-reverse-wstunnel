...
class WstServerReverse {
  constructor (secret, ssl) {
    ...
    const httpServer = ssl
      ? https.createServer(ssl, httpHandler)
    ...
    this.wsServerForControl.on('request', ((_this) => {
      return (request) => {
        ...
        if (uri.get('dst')) {
          ...
          isPortReachable(portTcp)
            .then((reachable) => {
              if (!reachable) {
                throw new Error('port is listened, aborted');
              }
          ...
          return (tcpConn) => {
            ...
            newWSTCPEmitter.on('created', ((_this) => {
              return (request) => {
                ...
                if (idConnection === id && (dayjs().second() - time) < 15) {
