  ...
  start(port) {
    ...
    this.wsServerForControl.on('request', ((_this) => {
      return (request) => {
        ...
        // parse to http uri
        const requestUri = url.parse(_.get(request, 'httpRequest.url'), true);
        if (requestUri.query.dst !== undefined) {
          // One TCP Server for each client WS Request
          request.tcpServer = new net.createServer();

          const remoteAddr = _.get(requestUri, 'query.dst', '');
        ...
        request.tcpServer.on('connection', ((_this) => {
          return (tcpConn) => {
            ...
            newWSTCPEmitter.on('created', ((_this) => {
              return (request) => {
                const stcpUri = url.parse(_.get(request, 'httpRequest.url'), true);
                if (idConnection === _.get(stcpUri, 'query.id')) {
