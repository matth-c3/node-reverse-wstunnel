...
class WstServerReverse {
  ...
  start(){
    this.httpServer.listen(this.port, function() {
      logger(`Server is listening on port: ${this.port}`);
      return
    }.bind(this));
    ...
    this.wsServerForControll.on('request', (function(_this){
      ...
      if (uri.query.dst !== undefined) {
        ...
        logger('${new Date()}Created TCP server on port: portTcp);
        ...
        request.wsConnectionForControll.on('close', function(reasonCode, description) {
          logger(`WebSocket Control Peer ${request.wsConnectionForControll.remoteAddress} disconnected for: ${description}`);
