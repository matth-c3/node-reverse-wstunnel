...
class WstServerReverse {
  constructor(secret) {
    ...
    this.wsClientForControl = wsServerForControl;
  }
  ...
  start(port) {
    ...
    this.wsServerForControl.on('request', (function(_this){
      return function(request) {
        ...
        const requestUri = url.parse(request.httpRequest.url, true);
        if (requestUri.query.dst !== undefined) {
          // One TCP Server for each client WS Request
          request.tcpServer = new net.createServer();

          const remoteAddr = requestUri.query.dst;
        ...
        request.tcpServer.on('connection', (function(_this){
          return function(tcpConn) {
            ...
            newWSTCPEmitter.on('created', (function(_this){
              return function(request) {
                const stcpUri = url.parse(request.httpRequest.url, true);
                if (idConnection === stcpUri.query.id) {
