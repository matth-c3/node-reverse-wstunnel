...
class wst_server_reverse {
    ...
    start(){
        this.httpServer.listen(this.port, function() {
            console.log('[wst_server_reverse.start]'+(new Date())+ 'Server is listening on port: ' + this.port);
            return
        }.bind(this));
        ...
        this.wsServerForControll.on('request', (function(_this){
            ...
            if (uri.query.dst != undefined){
                ...
                console.log('[wst_server_reverse.start]'+(new Date())+'Created TCP server on port: '+portTcp);
                request.wsConnectionForControll = request.accept('tunnel-protocol', request.origin);
          

                console.log('[wst_server_reverse.start]'+(new Date())+'WS Connection for Control Created');

                request.wsConnectionForControll.on('close', function(reasonCode, description) {
                    console.log('[wst_server_reverse.start]'+(new Date())+ 'WebSocket Control Peer '+
                        request.wsConnectionForControll.remoteAddress+
                        ' disconnected for:"'+description+'"');
                    console.log('[wst_server_reverse.start]'+(new Date())+'Close TCP server on port '+portTcp);
            ...
            //REQUEST FOR WS SOCKET USED FOR DATA
                ...
                console.log('[wst_server_reverse.start]'+(new Date())+'Request for Data WS Socket');
