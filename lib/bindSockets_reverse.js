    ...
    start(){
        ...
        this.wsconn.on('message', function(message) {
            ...
            if (message.type === 'utf8') {
                logger('bindSocket' + (new Date()) + ':\tError, text messages is not supported by Reverse WS-Tunnel ');
                return;
            }
        ...
        this.wsconn.on("overflow", function() {
            logger('[bindSocket]' + (new Date()) + ':\tTCP connection paused for WS OVERFLOW');
        ...
        this.wsconn.on("error", function(err) {
            logger('[bindSocket]' + (new Date()) + ':\tWebSocket Error ' + err);
        return;
        }.bind(this));

        this.wsconn.on('close', function(reasonCode, description) {
            logger('[BindSockets]' + (new Date()) + ':\tWebSocket Peer ' + this.wsconn.remoteAddress + ' disconnected for:\"'+description+'\"');
        ...
        this.tcpconn.on("drain", function() {
            this.wsconn.socket.resume();
            logger('[bindSocket]' + (new Date()) + ':\tWS resumed');
            return this.wsconn.__paused = false;
        }.bind(this));

        this.tcpconn.on("data", function(buffer) {
            logger('[BindSockets]'+ (new Date()) + ':\tTCP DATA ');
            return this.wsconn.sendBytes(buffer);
        }.bind(this));

        this.tcpconn.on("error", function(err) {
            logger('[BindSockets]'+ (new Date()) + ':\tTCP Error ' + err);
            return this.tcpconn.destroy();
        }.bind(this));

        this.tcpconn.on("close", function() {
            logger('[BindSockets]'+ (new Date()) + ':\tTCP connection Close');
